...
Herdr delegation is opt-in: delegate only when the user authorizes it. Unless the user supplies a larger cap, keep at most two active workers. Give each a fresh, bounded task; writers work in their own worktrees; the orchestrator retains review, merge, and authority. Choose the roster per task and verify provider, model, and effort from the actual session—do not hardcode campaign-specific models here.

Keep a durable record for every task: dispatch ID, named agent and pane, session-file path, branch/base/head, exact scope, state/result artifact, and independently verified completion. `idle` and `done` are lifecycle states, not correctness; `blocked` is not success. Reuse a historical agent only for a tightly scoped fix, and never put two writers in one worktree.

Roll through ready work: dispatch a task, then run `herdr agent wait <target> --timeout <bounded-ms>` and inspect only that worker's short result before refilling the free slot. Use wait-any only when the installed CLI/API actually exposes it. Otherwise use two concurrent CLI waits in a local non-LLM wrapper, or wait for the shortest/critical task first and check the other worker on timeout or settlement. Do not invent flags. A tool timeout is not worker failure and must not cancel the worker.

The orchestrator stays in its active tool loop while waiting; do not end the response expecting a blocking wait to wake a dormant model later. Blocking wait avoids LLM polling, but is neither a daemon nor push notification after the assistant turn ends. User interruption and new instructions take precedence; never ignore prompts or auto-approve actions.

Protect cache locality: keep the orchestrator model, session, instructions, and tools stable; use compact bounded worker reports; read changed hunks rather than full transcripts; delegate bulk code/tests; avoid repeated status polling and keepalive model calls. Do not promise that `PI_CACHE_RETENTION` changes Codex behavior; verify provider support before tuning. Keep session paths and checkpoints for canonical `skill-cost`, not UI cost, and never embed task or project secrets in this global skill.

Archive finished workers intact, never by killing them. Use the installed move syntax, choosing the layout direction explicitly:

```bash
herdr pane move <PANE_ID> --tab <ARCHIVE_TAB_ID> --split down --no-focus
```

Use `--split right` when that is the appropriate archive layout.
...
**Never use `pane send-text` with literal `\n` or `\r\n` escapes in shell strings** — single-quoted `'cmd\n'` passes the literal backslash and character `n` to the terminal instead of pressing Enter. Always use `herdr pane run <PANE_ID> "<command>"` for running commands in a pane.
